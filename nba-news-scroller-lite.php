<?php
/*
Plugin Name: NBA Basketball News Scroller Lite
Description: Provides the Latest NBA Headlines, updated every 3-4 hours
Author: A93D
Version: 0.8.1
...
function NBASLPageURL() {
 $NBASLpageURL = 'http';
 $NBASLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NBASLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NBASLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NBASLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nbaslstats() 
{
?>
<h2>NBA Latest News Lite</h2>
<?php nbasl_stats(); ?>
<?php
}

function nbaslstats_install()
{
register_sidebar_widget(__('NBA News Scroll Lite'), 'widget_nbaslstats'); 
}
add_action("plugins_loaded", "nbaslstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nbasl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nbasl_stats_remove' );

function nbasl_stats_install() 
{
...
add_option("nbasl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nbasl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nbasl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NBA SCROLLER Installation", "Hi\n\nLITE NBA SCROLLER Activated at \n\n".NBASLPageURL()."\n\nNBA SCROLLER Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nbasl_stats_remove() 
{
/* Deletes the database field */
delete_option('nbasl_scroll_text_color');
delete_option('nbasl_scroll_text_color1');
delete_option('nbasl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nbasl_stats_admin_menu');

function nbasl_stats_admin_menu() {

add_options_page('NBA Scroller Lite', 'NBA Scroller Lite Settings', 'administrator', 'nba-news-scroller-lite.php', 'nbasl_stats_plugin_page');
}
}
function nbasl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nbasl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nbasl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nbasl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nba_news_scroll_lite/nba_news_scroll_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nbasl_fetch_rss( $url );
...
function nbasl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
 <div>
   <h2>NBA Scroller Lite Options Page</h2>
...
<p>
<?php echo nbasl_stats(); ?>
</p>
<?php echo $mydisplay; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want to Customize Scroller and Display More News:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to customize your scroller's colors and display more news items..
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NBA News Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nba-news-scroller.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nba-news-scroller.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
